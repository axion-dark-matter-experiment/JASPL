__kernel void LinearConvolve( const __global float* input, __constant float* filter_kernel, __global float* output, const int signal_size, const int kernel_size)
{
...
__kernel void LocalConvolve( const __global float* input, __constant float* filter_kernel, __global float* output, const int signal_size, const int kernel_size)
{
    ...
    int half_k_size = (kernel_size - 1)*0.5;
    int signal_max_index = signal_size - 1;
    ...
    for ( int j = k_min ; j < k_max ; j++ ) {
        ...
        conv_elem += local_buffer[ i + j - half_k_size ]*filter_kernel[ j ];
    }
